...
def _cat(x: list[Tensor], data_type: str) -> Tensor:
    ...
    for t in x:
        if np.isscalar(t) and not isinstance(t, str):
            lst.append(paddle.to_tensor([t], data_type))
        else:
            lst.append(paddle.to_tensor(t, data_type))
...
@window_function_register.register()
def _kaiser(m: int, beta: float, sym: bool = True, dtype: str = "float64") -> Tensor:
    ...
    m = paddle.to_tensor(m, dtype=dtype)
    alpha = (m - 1) / 2.0
    w = paddle.i0(beta * paddle.sqrt(1 - ((n - alpha) / alpha) ** 2.0)) / paddle.i0(beta)
...
@window_function_register.register()
def _general_cosine(m: int, a: list[float], sym: bool = True, dtype: str = "float64") -> Tensor:
...
@window_function_register.register()
def _general_hamming(m: int, alpha: float, sym: bool = True, dtype: str = "float64") -> Tensor:
...
@window_function_register.register()
def _taylor(m: int, nbar=4, sll=30, norm=True, sym: bool = True, dtype: str = "float64") -> Tensor:
    ...
    for mi in range(len(ma)):
        number = signs[mi] * paddle.prod(1 - m2[mi] / s2 / (a**2 + (ma - 0.5) ** 2))
        if mi == 0:
            denom = 2 * paddle.prod(1 - m2[mi] / m2[mi + 1 :])
        elif mi == len(ma) - 1:
            denom = 2 * paddle.prod(1 - m2[mi] / m2[:mi])
        else:
            denom = 2 * paddle.prod(1 - m2[mi] / m2[:mi]) * paddle.prod(1 - m2[mi] / m2[mi + 1 :])
    ...
    w_result = w(paddle.arange(0, m, dtype=dtype))

    # normalize (Note that this is not described in the original text [1])
    if norm:
        scale = 1.0 / w((m - 1) / 2)
        w_result *= scale
    w_result = w_result.squeeze()
    return _truncate(w_result, needs_trunc)
...
def _tukey(m: int, alpha=0.5, sym: bool = True, dtype: str = "float64") -> Tensor:
    ...
    w3 = 0.5 * (1 + paddle.cos(math.pi * (-2.0 / alpha + 1 + 2.0 * n3 / alpha / (m - 1))))
...
@window_function_register.register()
def _gaussian(m: int, std: float, sym: bool = True, dtype: str = "float64") -> Tensor:
...
@window_function_register.register()
def _exponential(m: int, center=None, tau=1.0, sym: bool = True, dtype: str = "float64") -> Tensor:
...
def _bohman(m: int, sym: bool = True, dtype: str = "float64") -> Tensor:
    ...
    w = (1 - fac) * paddle.cos(math.pi * fac) + 1.0 / math.pi * paddle.sin(math.pi * fac)
    w = _cat([0, w, 0], dtype)
...
def _apply_window_postprocess(
    ...
    *,
    ...
    if in_dynamic_or_pir_mode():
        device = _get_paddle_place(device) if device is not None else _current_expected_place()
        if (
            pin_memory
            ...
            if isinstance(device, core.CUDAPlace) or (isinstance(device, core.Place) and device.is_gpu_place()):
                device = core.CUDAPinnedPlace()
            elif isinstance(device, core.XPUPlace) or (isinstance(device, core.Place) and device.is_xpu_place()):
...
def kaiser_window(
    ...
    *,
    ...
    """
    w = get_window(("kaiser", beta), window_length, fftbins=periodic, dtype=dtype)
    return _apply_window_postprocess(
        w,
        layout=layout,
        device=device,
