...
class TestFeatures(unittest.TestCase):
    ...
    def test_log_melspect(
        ...
        feature_extractor = compat_audio.features.LogMelSpectrogram(
            sr=sr,
            n_fft=n_fft,
            hop_length=hop_length,
            window=window_str,
        ...
        # Paddle vs librosa have known numerical differences in mel filter bank / STFT
        np.testing.assert_allclose(feature_librosa, feature_layer, atol=0.25, rtol=1e-3)
    ...
    def test_mfcc(self, sr: int, n_fft: int, n_mfcc: int, n_mels: int, dtype: str):
        ...
        x = paddle.to_tensor(self.waveform, dtype=dtype).unsqueeze(0)  # Add batch dim.
        feature_extractor = compat_audio.features.MFCC(
            sr=sr,
            n_mfcc=n_mfcc,
            n_fft=n_fft,
            hop_length=64,
        ...
        np.testing.assert_allclose(feature_layer_mfcc, feature_librosa, rtol=1e-1)
